...
class Visitor(AbstractVisitor):
    ...
    def visitTruncate(self, command):
        ...
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}<Truncate pos = {self.aux_printer.pos_command}>")
        self.aux_printer.add_output_sql(f"TRUNCATE TABLE ")
        command.table.accept(self)
        self.aux_printer.dec_tab()
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}</Truncate>")
        self.aux_printer.dec_tab()
    ...
    def visitCreateDatabase(self, command):
        self.aux_printer.inc_tab()
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}<CreateDatabase pos = {self.aux_printer.pos_command}>")
        self.aux_printer.add_output_sql(f"CREATE DATABASE ")
        command.database.accept(self)
        self.aux_printer.dec_tab()
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}</CreateDatabase>")
        self.aux_printer.dec_tab()
        self.aux_printer.add_output_sql(f";\n\n")
    ...
    def visitDelete(self, delete):
        self.aux_printer.inc_tab()
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}<Delete pos = {self.aux_printer.pos_command}>")
        self.aux_printer.add_output_sql(f"DELETE FROM ")
        delete.table.accept(self)
        if delete.where is not None:
            self.aux_printer.add_output_xml(
                f"{self.aux_printer.indent()}<Where>")
            self.aux_printer.add_output_sql(f"\nWHERE ")
        ...
        self.aux_printer.dec_tab()
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}</Delete>")
        self.aux_printer.dec_tab()
        self.aux_printer.add_output_sql(f";\n\n")
    ...
    def visitDropDatabase(self, command):
        self.aux_printer.inc_tab()
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}<DropDatabase pos = {self.aux_printer.pos_command}>")
        self.aux_printer.add_output_sql(f"DROP DATABASE ")
        command.database.accept(self)
        self.aux_printer.dec_tab()
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}</DropDatabase>")
        self.aux_printer.dec_tab()
        self.aux_printer.add_output_sql(f";\n\n")
    ...
    def visitDropTable(self, command):
        self.aux_printer.inc_tab()
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}<DropTable pos = {self.aux_printer.pos_command}>")
        self.aux_printer.add_output_sql(f"DROP TABLE ")
        command.table.accept(self)
        self.aux_printer.dec_tab()
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}</DropTable>")
        self.aux_printer.dec_tab()
        self.aux_printer.add_output_sql(f";\n\n")
    ...
    def visitSelect(self, select):
        self.aux_printer.inc_tab()
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}<Select pos = {self.aux_printer.pos_command}>")
        self.aux_printer.add_output_sql(f"SELECT ")
        self.aux_printer.inc_tab()
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}<Object>{select.table.name}</Object>")
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}<Columns>")
        self.aux_printer.inc_tab()
        ...
        self.aux_printer.dec_tab()
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}</Select>")
        self.aux_printer.dec_tab()

    def visitSelectAll(self, _):
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}<SelectAll/>")
        self.aux_printer.add_output_sql(f"*")
    ...
    def visitColumns(self, columns):
        ...
        for i, col in enumerate(columns_list):
            ...
            self.aux_printer.add_output_xml(
                f"{self.aux_printer.indent()}<Column>")
            col.accept(self)
            self.aux_printer.dec_tab()
            self.aux_printer.add_output_xml(
                f"{self.aux_printer.indent()}</Column>")
    ...
    def visitExpressionAri(self, expression):
        self.aux_printer.inc_tab()
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}<ExpressionAri>")
        expression.left.accept(self)
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}<Operator>{expression.operator}</Operator>")
    ...
    def visitExpressionComparison(self, expression):
        self.aux_printer.inc_tab()
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}<ExpressionComparison>")
        expression.left.accept(self)
        self.aux_printer.add_output_xml(
            f'{self.aux_printer.indent()}<Operator>"{expression.operator}"</Operator>')
        self.aux_printer.add_output_sql(f" {expression.operator} ")
    ...
    def visitExpressionBool(self, expression):
        self.aux_printer.inc_tab()
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}<ExpressionBool>")
        expression.left.accept(self)
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}<Operator>{expression.operator}</Operator>")
        self.aux_printer.add_output_sql(f" {expression.operator} ")
    ...
    def visitExpressionNullCheck(self, expression):
        self.aux_printer.inc_tab()
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}<ExpressionNullCheck>")
        expression.expression.accept(self)
        if expression.is_not:
            self.aux_printer.add_output_xml(
                f"{self.aux_printer.indent()}<Operator>IS NOT NULL</Operator>")
            self.aux_printer.add_output_sql(f" IS NOT NULL")
        else:
            self.aux_printer.add_output_xml(
                f"{self.aux_printer.indent()}<Operator>IS NULL</Operator>")
    ...
    def visitInsert(self, insert):
        self.aux_printer.inc_tab()
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}<Insert pos = {self.aux_printer.pos_command}>")
        ...
        self.aux_printer.add_output_sql(f"\nVALUES (")
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}<Parameters>")
        self.aux_printer.inc_tab()
        for i, param in enumerate(insert.parameters):
            if i > 0:
                self.aux_printer.add_output_sql(", ")
            self.aux_printer.add_output_xml(
                f"{self.aux_printer.indent()}<Parameter>")
            param.accept(self)
            self.aux_printer.dec_tab()
            self.aux_printer.add_output_xml(
                f"{self.aux_printer.indent()}</Parameter>")
        ...
        self.aux_printer.dec_tab()
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}</Insert>")
        self.aux_printer.dec_tab()
    ...
    def visitAssignmentUpdate(self, update):
        ...
        self.aux_printer.add_output_sql(" = ")
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}<Assignment>=</Assignment>")
        self.aux_printer.dec_tab()
    ...
    def visitUpdate(self, update):
        self.aux_printer.inc_tab()
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}<Update = {self.aux_printer.pos_command}>")
        ...
        self.aux_printer.add_output_sql(f";\n\n")
        self.aux_printer.add_output_xml(
            f"{self.aux_printer.indent()}</Update>")
        self.aux_printer.dec_tab()
